...
void csv_test()
{
    ...
    for (float thres = min_thres; thres < max_thres; thres += step_thres)
    {
        ...
        for (uint32_t iter = 0; iter <= iter_per_thres; ++iter)
        {
            ...
            {
                float round_time = insert_hashtable(pHashTable, insert_kvs.data() + i * num_inserts_per_batch, num_inserts_per_batch) + delete_hashtable(pHashTable, delete_kvs.data() + i * num_deletes_per_batch, num_deletes_per_batch);
                if (iter == 0)
                {
